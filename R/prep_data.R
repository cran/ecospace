...
prep_data <- function(ecospace, Smax) {
  if (class(ecospace) != "ecospace")
    stop("use create.ecospace() to create a formal ecospace framework.\n")
  ...
  char.names <- sapply(seq, function(seq)
    ecospace[[seq]]$char)
  cs <-
    sapply(seq, function(seq)
      ncol(ecospace[[seq]]$char.space) - 3)
  for (ch in 1:nchar) {
    if (ecospace[[ch]]$type == "numeric" |
        ecospace[[ch]]$type == "ord.num")
      for (s in seq_len(cs[ch]))
        pre.data <- as.data.frame(cbind(pre.data, numeric(Smax)))
      if (ecospace[[ch]]$type == "factor")
        for (s in seq_len(cs[ch]))
          pre.data <-
            as.data.frame(cbind(
              pre.data, factor(Smax, ordered = FALSE, levels = ecospace[[ch]]$allowed.combos)))
        if (ecospace[[ch]]$type == "ord.fac")
          for (s in seq_len(cs[ch]))
            pre.data <-
              as.data.frame(cbind(
                pre.data, factor(Smax, ordered = TRUE, levels = ecospace[[ch]]$allowed.combos)))
  }
  pre.data <- pre.data[, 2:ncol(pre.data)]
  if (length(char.names) == length(state.names)) {
    colnames(pre.data) <- char.names
  } else {
    colnames(pre.data) <- state.names
  }
  return(pre.data)
}
